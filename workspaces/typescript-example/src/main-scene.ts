...
export default class MainScene extends Scene {
  ...
  create() {
    ...
    // Create some Matter-enabled physics game objects.
    ...
    const emoji3 = this.matter.add.image(350, 350, "emoji", "1f4a9", bodyOptions);
    ...
    this.matterCollision.addOnCollideStart({
      ...
      callback: ({ gameObjectA, gameObjectB }) => {
        // These fields *can* be undefined depending on the inputs, so use the non-null assertion
        // operator when we know more than the type system.
    ...
    // Kill poop emoji on collision with lava.
    ...
    this.input.on(
      "drag",
      (pointer: Input.Pointer, gameObject: Physics.Matter.Sprite, x: number, y: number) =>
        gameObject.setPosition(x, y)
    );
    this.input.on("dragstart", (pointer: Input.Pointer, gameObject: Physics.Matter.Sprite) =>
      gameObject.setStatic(true)
    );
    this.input.on("dragend", (pointer: Input.Pointer, gameObject: Physics.Matter.Sprite) =>
